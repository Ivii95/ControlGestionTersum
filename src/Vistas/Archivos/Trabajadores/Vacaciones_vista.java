...
import Modelo.Repository.VacacionesRepository;
 ...
 * @author dwigh
 */
public final class Vacaciones_vista extends javax.swing.JFrame {

    VacacionesRepository repoFalta;
    VacacionesRepository repoFaltaTotal;
    ...
    public Vacaciones_vista(Centro centro) {
        ...
        repoFalta.rellenarTablaByVacacionesDeUnCentro(tabla_faltas, centroFaltas.getCodigo());
    }

    public Vacaciones_vista(Trabajador trabajador) {
    ...
    private void iniciarOtrosComponentes() {
        repoFalta = new VacacionesRepository();
        repoFaltaTotal = new VacacionesRepository();
        initComponents();
        ponAyuda();
        pulsarX();
    ...
    private void initComponents() {
        ...
        panelRect1.add(panelCurves1, new org.netbeans.lib.awtextra.AbsoluteConstraints(0, 6, 1380, 740));

        getContentPane().add(panelRect1, new org.netbeans.lib.awtextra.AbsoluteConstraints(-8, -10, -1, 740));
    ...
    private void btn_añadir_faltasActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_btn_añadir_faltasActionPerformed
        try {
            ...
            //PREGUNTAMOS AL USUARIO SI QUIERE CREAR UN PLUS PARA CUBRIR LAS VACACIONES DEL TRABAJADOR.
            int input = JOptionPane.showConfirmDialog(this,
                    "¿Quieres añadir un trabajador para cubrir estas vacaciones?", "Crear un PLUS", JOptionPane.YES_NO_OPTION);
            if (input == 0) {
                ...
                if (opcion != null && !opcion.equals("")) {
                    Pluses_vista inci = new Pluses_vista(tr.getByNombre(opcion), falta);
    ...
    private void rellenardatos(int filaSeleccionada) {
        ...
        int id = (int) tabla_faltas.getValueAt(filaSeleccionada, 0);
        Vacaciones falta = repoFalta.getById(id);
